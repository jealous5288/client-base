$(document).ready(function () {
    ...
    var eles = {};
    eles.name = $("#titleText");
    eles.startupProcessSql = $("#startupProcessSql"); // 前处理
    eles.finishProcessSql = $("#finishProcessSql"); // 后处理
    eles.nodeSort = $("#nodeSort"); // 说明
    eles.transactUser = $("#transactUser"); // 办理人
    ...
    if (node) {
        ...
        if (pro.transactUser) {
            eles.transactUser.val(pro.transactUser.name).data('colony', pro.transactUser);
        }
    }
    $("#save").omButton({icons: {left: '/medias/images/baseModel/button/accept.gif'}}).click(function () {
        if (pro) {
            ...
            if (obj != null) {
                ...
                if (text != null) {
                    node.textWrap2(text, 60, newName,36);
                    text.attr("title", newName);
                }
            }
            var transactUser = eles.transactUser.data('colony');
            if (transactUser) {
                pro.transactUser = transactUser;
            }
    ...
    setTimeout(function () {
        $(".colony_iframe").each(function (i) {
            var ifr = this;
//                ifr.src = "${createLink(controller: 'workflowSetting', action: 'sysColonySetting')}";
            ifr.src = "/workflow/designTools/sysColonySetting";
            var id = ifr.id;
            var input = $("#" + id.split("_").shift());
            ifr.afterLoad = function () {
                var win = ifr.contentWindow;
                if (input.data('colony'))
                    win.setColony(input.data('colony'), input);
                var doc = ifr.contentDocument || ifr.contentWindow.document;
                $("#ok", doc).click(function () {
                    if (win.getCurrentColony) {
                        var json = win.getCurrentColony(function (name) {
                            input.val(name);
                        });
                        if (json == null) {
                            alert("请选择群体！");
                        ...
                        input.data('colony', json);
                    }
                });
            }
            if (ifr.attachEvent) {
                ifr.attachEvent("onload", ifr.afterLoad);
            } else {
                ifr.onload = ifr.afterLoad;
            }
        });
    }, 100)
});
